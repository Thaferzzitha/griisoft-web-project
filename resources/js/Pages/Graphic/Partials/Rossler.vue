<template>
  <div>
    ...
    <div class="container mx-auto p-4">
      <label v-if="$page.props.auth.user" for="title" class="block mb-1 dark:text-gray-300">Título</label>
      <input v-if="$page.props.auth.user" v-model="title" type="text" id="title"
        class="w-full border-gray-300 border rounded-md p-2 mb-1" :disabled="isReadOnly" />
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div>
          <label for="pointsNumber" class="block mb-1 dark:text-gray-300">Número de Puntos</label>
          <input v-model="pointNumber" type="number" id="pointsNumber"
            class="w-full border-gray-300 border rounded-md p-2" step="1" :disabled="isReadOnly" />
        </div>
        <div>
          <label for="stepSize" class="block mb-1 dark:text-gray-300">Número de Pasos</label>
          <input v-model="stepSize" type="number" id="stepSize" class="w-full border-gray-300 border rounded-md p-2"
        ...
        <div>
          ...
          <input v-model="x" type="number" id="x" class="w-full border-gray-300 border rounded-md p-2" step="0.1"
            :disabled="isReadOnly" />
        </div>
        <div>
          <label for="y" class="block mb-1 dark:text-gray-300">Variable Y</label>
          <input v-model="y" type="number" id="y" class="w-full border-gray-300 border rounded-md p-2" step="0.1"
            :disabled="isReadOnly" />
        </div>
        <div>
          <label for="z" class="block mb-1 dark:text-gray-300">Variable Z</label>
          <input v-model="z" type="number" id="z" class="w-full border-gray-300 border rounded-md p-2" step="0.1"
            :disabled="isReadOnly" />
        </div>
      </div>
    </div>
    ...
    <div class="flex space-x-4 dark:text-gray-300">
      <label class="flex items-center">
        <input type="radio" class="form-radio" name="view" value="view3d" v-model="plotView">
        <span class="ml-2">Vista 3D</span>
      </label>
      <label class="flex items-center">
        <input type="radio" class="form-radio" name="view" value="viewxy" v-model="plotView">
        <span class="ml-2">Vista XY</span>
      </label>
      <label class="flex items-center">
        <input type="radio" class="form-radio" name="view" value="viewxz" v-model="plotView">
        <span class="ml-2">Vista XZ</span>
      </label>
      <label class="flex items-center">
        <input type="radio" class="form-radio" name="view" value="viewyz" v-model="plotView">
    ...
    <div class="my-10">
      ...
      <PrimaryButton v-if="$page.props.auth.user && !isReadOnly" @click="onSavePlot" class="ml-4"> Guardar
      </PrimaryButton>
    </div>
